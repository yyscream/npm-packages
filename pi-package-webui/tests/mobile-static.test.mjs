...
assert.match(app, /function splitThinkingFormatText\(text, \{ streaming = false \} = \{\}\)[\s\S]*?THINKING_FORMAT_CLOSE_TAG_REGEX[\s\S]*?finalText: stripThinkingFormatOutputSeparator\(rest\)/, "tagged <think> output should split thinking text from final response text");
