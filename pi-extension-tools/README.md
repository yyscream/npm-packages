...
- `/tools` — open a TUI to enable/disable individual tools.
...
Tool choices are stored globally in `~/.pi/agent/tools.json` (or `$PI_CODING_AGENT_DIR/tools.json`) with both `active` and `inactive` tool lists. On startup, only `active` tools are restored from that file. If the file is missing, the extension falls back to the current session branch state, then Pi's current active tools.
